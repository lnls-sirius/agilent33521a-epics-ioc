...
where
    P: ServerProto<TcpStream>,
{
    fn from(connection: ConnectionFuture, protocol: Arc<Mutex<P>>) -> Self {
        Self {
            connection,
            protocol,
        }
...
where
    P: ServerProto<TcpStream>,
{
    fn advance(&mut self) -> Poll<P::Transport, Error> {
